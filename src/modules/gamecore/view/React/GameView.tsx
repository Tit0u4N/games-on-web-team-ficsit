...
const GameView: React.FC<GameViewProps> = ({presenter}) => {
    ...
    const [activeTutorialStep, setActiveTutorialStep] = React.useState<TutorialStep>(config.narratorBox.enabled ? TutorialStep.INTRODUCTION : TutorialStep.DISABLED);
    ...
    const getReactElementFromCurrentState = () => {
        switch (activeTutorialStep) {
            ...
            case TutorialStep.INTRODUCTION:
                return (
                    <SpeechBox>
                        <IntroductionSpeech speeches={speechTexts} onComplete={() => {
    ...
    return (
        <div>
            ...
            {(activeTutorialStep !== TutorialStep.DISABLED) &&
                <div className={
                    "fixed inset-0 bg-gray-600 bg-opacity-70 z-[45] flex items-center justify-center"
                }></div>}
