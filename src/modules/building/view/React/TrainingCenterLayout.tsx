...
export const TrainingCenterLayout: React.FC<TrainingCenterLayoutProps> = ({
    ...
    useEffect(() => {
        if (isOpen && !trainingCenterPresenter.buildingPresenter.hasTrainingCenterShownNarratorBox && config.narratorBox.enabled) {
    ...
    useEffect(() => {
        ...
        if (activeNarrationStep >= config.narratorBox.trainingCenterHighlightTutorialSpeech.length || !config.narratorBox.enabled || trainingCenterPresenter.buildingPresenter.hasTrainingCenterShownNarratorBox) return;
    ...
    return (
        <>
            {activeNarrationStep < config.narratorBox.trainingCenterHighlightTutorialSpeech.length && isNarrationVisible && (
                <>
                    <div className={
                        "fixed inset-0 bg-opacity-70 z-[45] flex items-center justify-center"
                    }></div>
            ...
            <Modal isOpen={isOpen && !hideModal} onClose={() => {
                        ...
                        <div className={'flex flex-col w-[37%] min-w-[500px]'} id={"AllAthletes"}>
                            {trainingCenter.sports.length > 0 && (
                                <>
                                        ...
                                        <div className="grid grid-cols-2">
                                            {trainingCenter.sports.map((sport: Sport, index: number) => (
                                                <div key={index} className="flex my-1">
                                                    <Image src={sport.iconPath} width={30} height={30}
                                                           alt={sport.name}/>
                                    ...
                                    <div id={"RoundTrainingBeforeChange"}>Rounds before sports
                                        change: {trainingCenter.rotation}</div>
                                </>
                            )}
