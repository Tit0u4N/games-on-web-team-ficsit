...
export const ClickableCard: React.FC<ClickableCardProps> = ({ character, trainingCenter, choice, isSelected, onSelect }) => {
    ...
    <Card key={character.id} radius={'lg'} className={cardClassName}>
      <CardBody>
        <div onClick={onSelect} className="w-full h-full">
          <CharacterStats character={character} trainingCenter={trainingCenter} choice={choice} />
        </div>
      </CardBody>
    </Card>
  );
